...
/// <summary>
/// Acceso a la tabla `techguessr-commit-sessions`. Análogo a SessionsRepository
/// pero para el modo CommitGuessr.
/// RoundsPlayed se serializa como un único atributo JSON (string).
/// </summary>
public sealed class CommitSessionsRepository(IAmazonDynamoDB client, string tableName)
{
    private static readonly JsonSerializerOptions JsonOptions = new() { PropertyNamingPolicy = null };

    public async Task CreateAsync(CommitSessionState session, CancellationToken ct = default)
    {
    ...
    public async Task<CommitSessionState?> GetAsync(string sessionId, CancellationToken ct = default)
    {
    ...
    /// <summary>
    /// Reemplaza el ítem completo de la sesión CommitGuessr, condicionado a
    /// que `version` siga siendo la misma (optimistic locking).
    /// </summary>
    public async Task SaveAsync(CommitSessionState session, long expectedVersion, CancellationToken ct = default)
    {
    ...
    private static Dictionary<string, AttributeValue> ToItem(CommitSessionState session) => new()
    {
    ...
    private static CommitSessionState FromItem(Dictionary<string, AttributeValue> item) => new(
        SessionId: item["sessionId"].S,
        UserId: item["userId"].S,
        Status: item["status"].S == "finished" ? SessionStatus.Finished : SessionStatus.InProgress,
        RoundsPlayed: JsonSerializer.Deserialize<List<CommitRoundRecord>>(item["roundsPlayed"].S, JsonOptions) ?? [],
